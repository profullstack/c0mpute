...
use anyhow::Result;
use quest_net::{ChunkSource, Loopback, Network};
...
impl Supervisor {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        if self.config.roles.contains(&quest_proto::Role::Gateway) {
            let bind: std::net::SocketAddr = self.config.gateway.bind.parse()?;
            let state = quest_gateway::GatewayState {
                store: self.store.clone(),
                net: self.net.clone(),
            };
            tokio::spawn(async move {
                if let Err(e) = quest_gateway::serve(state, bind).await {
