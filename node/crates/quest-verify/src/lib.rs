...
pub struct StorageChallenge {
    pub chunk_hash: quest_proto::Hash,
    pub offset: u32,
    pub length: u32,
}
...
impl StorageChallenge {
    ...
    pub fn expected_response(&self, full_chunk: &[u8]) -> Option<quest_proto::Hash> {
        let start = self.offset as usize;
        let end = start.checked_add(self.length as usize)?;
        if end > full_chunk.len() {
            return None;
        }
        Some(quest_proto::Hash::of(&full_chunk[start..end]))
    }
}
...
mod tests {
    ...
    fn challenge_response_matches_slice() {
        ...
        let chunk_hash = quest_proto::Hash::of(bytes);
        ...
        assert_eq!(expected, quest_proto::Hash::of(b"quick"));
    }
}
